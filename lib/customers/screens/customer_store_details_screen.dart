...
class _CustomerStoreDetailsScreenState
  ...
  void initState() {
    super.initState();
    _loadUserRating();
    ratingFuture = context.read<GlobalProvider>().getStoreRatingDetails(
      widget.store.id,
    );
  }
  ...
  Future<void> submitRating(int rating) async {
    ...
    try {
      await provider.rateStore(storeId: widget.store.id, rating: rating);

      final navCtx = navigatorKey.currentContext;
      if (navCtx != null) {
        ScaffoldMessenger.of(
          navCtx,
        ).showSnackBar(SnackBar(content: Text("Rating saved")));
      }

      if (!mounted) return;

      setState(() {
        ratingFuture = provider.getStoreRatingDetails(widget.store.id);
      });
    } catch (e) {
      final navCtx = navigatorKey.currentContext;
      if (navCtx != null) {
        ScaffoldMessenger.of(
          navCtx,
        ).showSnackBar(SnackBar(content: Text("Failed to save rating")));
      }
    }
  }
